...
PROEJCT_DATA_FASTQ_DIR=$PROJECT_DATA_DIR/fastqc
...
for fastq_file in $input; do
    ...
    qc_report="$PROEJCT_DATA_FASTQ_DIR"/"$(echo "$file_name" | cut -d '.' -f1)_fastqc.html"
    if [ -f "$qc_report" ]; then
        log "QC report already exists ($qc_report), skipping $fastq_file"
    else
        log "Running FastQC on $fastq_file"
        temp_output=$(mktemp)
        $fastqc -o "$PROEJCT_DATA_FASTQ_DIR" "$fastq_file" 2>&1 | tee "$temp_output"
        log $(cat $temp_output)
